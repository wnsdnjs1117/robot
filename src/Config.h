...
const int BUZZER_PIN = 5;  // 부저 핀 (Arduino tone() 사용)
...
const int FINISH_ENTRY_COUNTS  = 1500;  // FINISH 구역 진입 거리
const int SPIN_90_COUNTS       = 1200;  // 90도 회전 시 필요한 엔코더 카운트
const int CROSS_ALIGN_COUNTS   = 250;   // 교차로 감지 후 축 정렬 과전진 거리
const int ZONE_ENTER_COUNTS    = 2200;  // 교차로 ➔ 구역 안쪽 전진 진입 거리
const int ZONE_DEPTH_COUNTS    = 1500;  // 구역 내부 수직 관통 깊이 (탐색용)
const int ZONE_EXIT_REV_COUNTS = 2000;  // 구역 내부 → 교차로 후진 거리
