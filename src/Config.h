...
constexpr int SENSOR_REAR_LEFT =
    A1;  // 후방 아날로그 (차축 후방 24cm, 1000 count)
...
constexpr int START_ESCAPE_COUNTS = 1300;  // 스타트 선 밟은 후 추가 이탈 거리
constexpr int FINISH_ENTRY_COUNTS = 1500;  // FINISH 구역 진입 거리
constexpr int REAR_TO_AXLE_COUNTS =
    1000;  // 후방 센서 → 차축 거리 (24cm ≈ 1000 count)

// ── [4] 존 진입/탈출 거리 ────────────────────────────────────
constexpr int ZONE_ENTER_COUNTS = 2400;     // 노드7 전진 탈출 전용 (exitZone)
constexpr int ZONE_ENTER_EXTRA = 1200;      // 전진 진입: 라인 끊긴 후 추가 직진
constexpr int ZONE_DEPTH_EXTRA = 800;       // 후진 진입: 라인 끊긴 후 추가 후진
constexpr int ZONE_EXIT_REV_COUNTS = 2000;  // 구역 내부 → 교차로 후진 거리
constexpr int ZONE_FOLLOW_MAX =
    3500;  // 후진 라인 추종 최대 거리 (무한루프 방지)
constexpr int BLIND_DRIVE_MAX_COUNTS =
    5000;  // 블라인드 직진 안전 한계 (노드 간 최대 거리)
