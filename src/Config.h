 ...
 *
 * ■ 가·감속 규칙
 * RAMP_ACCEL_CM / RAMP_DECEL_CM 은 cruise 40 기준 거리(cm).
 * cruise 가 40이 아니면 거리만 × (speed / 40) 으로 스케일.
 *
 * ■ 방위(heading)
 * 0=북, 90=동, 180=남, 270=서
 * rotateToHeading() 은 최소 각도로 회전.
 *
 * ■ 정지
 * 목표 거리·각도 직전 stopMotors() → 모터 125 급제동.
 * ============================================================ */
 #ifndef CONFIG_H
 #define CONFIG_H
...
// ============================================================
 ...
 constexpr unsigned long MOTOR_VELOCITY_PID_MS = 10; // (수정) 5ms -> 10ms I2C 통신 분산
 // setWheelSpeeds() 엔코더 PID 갱신 주기 (ms)
 ...
 constexpr unsigned long LIFT_TICK_INTERVAL_MS = 15; // (수정) 5ms -> 15ms I2C 통신 분산
