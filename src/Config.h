...
// [3] 디버그 시리얼 출력
// ============================================================
#define ROBOT_DEBUG 1                               // 0이면 DPRINT* 매크로 전부 제거 (플래시 절약)
...
// [4] 엔코더 / 거리 변환
// ============================================================
constexpr float COUNTS_PER_CM = 3570.0f / 80.0f; // 80cm 주행 시 3570 카운트 (실측)
constexpr int toEncoderCounts(float cm) { return (int)(cm * COUNTS_PER_CM + 0.5f); }
...
constexpr bool INVERT_LINE_SENSORS  = false; // true = 센서 극성 반전 (흰 바탕·검은 선)
...
constexpr float LINE_THICKNESS_CM            =  1.0f;  // 선 두께 보정 (실제 2cm − 제동 여유 1cm)

// ============================================================
// [7] 존 진입 / 탈출 거리
// ============================================================
constexpr float DIST_CROSS_ALIGN_CM = DIST_AXIS_TO_FRONT_SENSOR_CM; // 교차로 중심 정렬 거리
...
inline ZoneMotionProfile getZoneProfile(int zone) {
  ...
  if (zone == 1) { p.exitForwardExtra = 35.0f; p.exitReverseExtra = 33.0f; }
  else if (zone == 3) { p.exitForwardExtra = 37.0f; p.exitReverseExtra = 35.0f; }
  // 5·6구역: 항상 전진 진입 → 후진 탈출만
  else if (zone == 5 || zone == 6) { p.exitReverseExtra = 28.0f; }
...
// [8] 주행 속도
// ============================================================
// 가감속 프로파일 (driveDistanceCm 전용)
constexpr int   RAMP_MIN_SPEED = 10;    // 가속 시작·감속 끝 최저 속도
constexpr float RAMP_ACCEL_CM  =  8.0f; // 가속 구간 (cm)
...
constexpr int SPIN_90_COUNTS = 1185;  // 90° 회전 엔코더 카운트 (실측)
...
constexpr int SPEED_BLIND_FWD = SPEED_OPEN_DRIVE_FWD;
constexpr int SPEED_BLIND_REV = SPEED_OPEN_DRIVE_REV;
