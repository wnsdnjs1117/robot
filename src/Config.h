...
// ── [0] 단위 변환 ────────────────────────────────────────────
// 24cm = 1000 counts (물리 측정값)
constexpr float COUNTS_PER_CM = 1000.0f / 24.0f;
...
constexpr int CROSS_ALIGN_COUNTS = CM(6.0f);    // 교차로 감지 후 축 정렬 과전진 (6cm)
constexpr int START_ESCAPE_COUNTS = CM(31.0f);  // 스타트 선 밟은 후 추가 이탈 거리 (31cm)
