...
constexpr float RAMP_DECEL_CM = 20.0f;  // 정지를 위한 감속 구간 거리
...
constexpr float LINE_KP_TRACK_7_9_SOFT = 1.8f;  // 메인 트랙(7-8-9) 라인 부드러운 조향 게인
constexpr float LINE_KP_TRACK_7_9_HARD = 2.5f;  // 메인 트랙(7-8-9) 라인 강한 조향 게인
...
struct ZoneMotionProfile {
  float entryForwardExtra = 34.0f;  // 라인 감지 후 박스 존 진입을 위한 추가 주행 거리 (전진)
...
constexpr float DIST_TRACK_13_TO_9_CM = 9990.0f;  // 13에서 9까지의 대각선 주행 거리
...
constexpr float DIST_TRACK_12_TO_9_CM = 9999.0f;  // 12 -> 9 배송 복귀 직진 거리
...
constexpr float LIFT_CARRY_LOW_CM = 14.0f;                // 1<->3·5<->6 운반 높이(장애물 없음)
