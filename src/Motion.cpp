...
static long encoderRemaining(DriveEncMark mark, long targetSpan) { return targetSpan - encoderTraveledSince(mark); }

static int iround(float v) { return (int)(v >= 0.0f ? v + 0.5f : v - 0.5f); }

DriveEncMark captureDriveEnc() { return {prizm.readEncoderCount(1), prizm.readEncoderCount(2)}; }
...
int rampMarkSpeed(DriveEncMark start, int maxSpeed) {
  return calcRampUpSpeed(encoderTraveledSince(start), rampAccelSpanCounts(maxSpeed), maxSpeed);
}
...
int decelMarkSpeed(DriveEncMark mark, long totalSpan, int cruiseSpeed) {
  ...
  if (totalSpan <= decelSpan) return calcRampDownSpeed(remaining, totalSpan, cruiseSpeed);
...
static void stopForTarget(int /*curSpeed*/) { stopMotors(); }
...
int calcRampUpSpeed(long traveledCounts, long accelCounts, int maxSpeed) {
  ...
  int speed = RAMP_MIN_SPEED + (int)((long)(maxSpeed - RAMP_MIN_SPEED) * traveledCounts / accelCounts);
...
int calcRampDownSpeed(long remainingCounts, long decelCounts, int startSpeed) {
  ...
  int speed = RAMP_MIN_SPEED + (int)((long)(startSpeed - RAMP_MIN_SPEED) * remainingCounts / decelCounts);
  return (speed < RAMP_MIN_SPEED) ? RAMP_MIN_SPEED : speed;
}

static int rampLimiterPrev = RAMP_MIN_SPEED;

void resetRampSpeedLimiter(int initialSpeed) { rampLimiterPrev = initialSpeed; }

int smoothRampSpeed(int targetSpeed) {
  if (targetSpeed > rampLimiterPrev + RAMP_MAX_SPEED_STEP) targetSpeed = rampLimiterPrev + RAMP_MAX_SPEED_STEP;
...
void setWheelSpeeds(int left, int right) {
  ...
  if (left != lastReqL || right != lastReqR) {
    outL = left;
    outR = right;
  }
  lastReqL = left;
...
static long spinDegToCounts(float deg) { return (long)((SPIN_90_COUNTS / 90.0f) * deg + 0.5f); }
...
static void spinMotorSpeeds(bool clockwise, int speed) {
  ...
  if (clockwise)
    prizm.setMotorSpeeds(-speed * 7, -speed * 7);
  else
    prizm.setMotorSpeeds(speed * 7, speed * 7);
}
...
void rotateByDegrees(int degrees, bool clockwise) {
  ...
  // [핵심 1] 동적 프로파일 비율 기반 분배 (짧은 회전에서도 감속 구간을 충분히 확보)
  ...
  if (targetCounts >= idealAccelSpan + idealDecelSpan) {
    ...
    // 부족할 경우 50:50이 아닌 설정된 가속/감속 비율에 맞춰 동적 분배
  ...
  while (true) {
    ...
    if (!skipLineTrim && !lineTrimmed && pos >= (long)(targetCounts * SPIN_LINE_TRIM_MIN_FRAC)) {
      ...
      if (oppositeOn && !oppositeWasOn) {
        ...
        // [핵심 2] 라인을 찾아 목표가 급단축될 때, 급정거(Jerk) 방지
        decelSpan = remaining;
        if (decelSpan <= 0) decelSpan = 1;
        accelSpan = 0;             // 즉시 감속 모드 돌입
        peakSpeed = lastCurSpeed;  // 방금 전 달리고 있던 속도를 최고점으로 삼아 자연스럽게 하강
    ...
    if (pos < accelSpan && !lineTrimmed) {
      ...
      curSpeed = peakSpeed;
      if (remaining <= decelSpan) curSpeed = min(curSpeed, calcRampDownSpeed(remaining, decelSpan, peakSpeed));
      if (remaining <= endDecelSpan) curSpeed = min(curSpeed, calcRampDownSpeed(remaining, endDecelSpan, peakSpeed));
    }

    curSpeed = smoothRampSpeed(curSpeed);
    lastCurSpeed = curSpeed;  // 현재 속도 기록
...
void traceLineForward(int fl, int fc, int fr, int rl, int rc, int rr, int baseSpeed, float kpSoft, float kpHard) {
  ...
  if (fl && !fc && !fr) {
    ...
    posFront = (lineTraceLastEdge == 1) ? -3 : ((lineTraceLastEdge == 2) ? 3 : 0);
  }
  ...
  if (fl == 1 && fc == 0 && fr == 0 && rl == 1 && rc == 0 && rr == 0) {
    leftSpeed -= EDGE_SYNC_GAIN;
    rightSpeed += EDGE_SYNC_GAIN;
...
void traceLineReverse(int rl, int rc, int rr, int fl, int fc, int fr, int baseSpeed) {
  ...
  if (rl && !rc && !rr) {
    ...
    posRear = (lineTraceLastEdge == 1) ? -3 : ((lineTraceLastEdge == 2) ? 3 : 0);
  }
...
void driveDistanceCm(float cm, int speed, bool stopAtEnd) {
  ...
  // [직진 동적 프로파일 수정] 짧은 직진에서도 50:50이 아니라 올바른 가감속 비율로 할당
...
void driveOverLinesAndAlign(int lineCount, float alignCm, int speed, bool stopAtEnd) {
  ...
  if (stopAtEnd && plannedLegSpanCm > 0.0f) correctTrackLegOvershoot(motionStart, plannedLegSpanCm);
}